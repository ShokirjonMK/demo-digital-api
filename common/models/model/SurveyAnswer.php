            ...
            if (isset($exam)) {
                if (isset($post['answers'])) {
                    ...
                    if (!isJsonMK($post['answers'])) {
                        ...
                        $newOrUpdateModelOne = self::findOne([
                            'exam_id' => $examId,
                            'created_by' => $post['user_id'],
                        ]);

                        if (!$newOrUpdateModelOne) {
                            $newOrUpdateModelOne = new self();
                        }


                        $dataAnswer = array_map('intval', json_decode($post['answers'], true));
                        $sumAnswer = array_sum($dataAnswer);
                        $countAnswer = count($dataAnswer);

                        // Calculate the average
                        $averageBall = (($countAnswer > 0 ? $sumAnswer / $countAnswer : 0));


                        $newOrUpdateModelOne->user_id = $post['user_id'];
                        $newOrUpdateModelOne->student_id = $post['student_id'];
                        $newOrUpdateModelOne->exam_id = $examId;
                        $newOrUpdateModelOne->edu_semestr_subject_id = $exam->edu_semestr_subject_id;
                        $newOrUpdateModelOne->subject_id = $exam->eduSemestrSubject->subject_id;
                        $newOrUpdateModelOne->data = (array)json_decode($post['answers']);
                        $newOrUpdateModelOne->ball = $averageBall;

                        // dd($newOrUpdateModelOne);
                        // if (!($newOrUpdateModelOne->validate())) {
                        //     $errors[] = $newOrUpdateModelOne->errors;
                        //     $transaction->rollBack();
                        //     return simplify_errors($errors);
                        // }
                        if (!$newOrUpdateModelOne->save(false)) {
                            $errors['answers'] = [_e('saving error')];
                        }
