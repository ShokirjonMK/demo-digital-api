...
trait ApiActionTrait11
{
    ...
    public function afterAction($action, $result)
    {
        ...
        $myfile = fopen(\Yii::getAlias('@api/web/logs') . "/log-" . date("Y-m-d") . ".json", "a");
        // current_user_id()
    ...
    //         // $myfile = fopen(\Yii::getAlias(LOGS_PATH . '/dairly') . DIRECTORY_SEPARATOR. "log-" . date("Y-m-d") . ".json", "a");
    //         // fwrite($myfile, "," . json_encode($data));
    //         // fclose($myfile);

    //         $logFilePath = Yii::getAlias(LOGS_PATH . '/dairly') . DIRECTORY_SEPARATOR . "log-" . date("Y-m-d") . ".json";
